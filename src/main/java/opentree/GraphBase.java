...
public abstract class GraphBase {
    ...
    public Node findTaxNodeByName(final String name) throws TaxonNotFoundException, MultipleHitsException {
     ...
     /**
     * NEW: Search by OTT_NODE_ID, either OTTID (from taxonomy) or node ID from the node registry
     * @param ottNodeID, will be of form ott54234 or node7646
     * @return node
    ...
    public Node findGraphNodeByOTTNodeID(final String ottNodeID) throws MultipleHitsException, TaxonNotFoundException {
        ...
        try {
            ...
            throw new MultipleHitsException(ottNodeID);
        } finally {
            hits.close();
        }
        if (firstNode == null) {
            throw new TaxonNotFoundException(NodeProperty.OT_NODE_ID.propertyName + " = " + String.valueOf(ottNodeID));
        }
        return firstNode;
    }
    ...
    // *** if we want to keep this, have to be safer with which nodes to delete
    /**
