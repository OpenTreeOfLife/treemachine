...
public class TreePrinter {
	...
	public void reportASCII(JadeTree tree, PrintWriter out) {
		printASCII(tree, out);
		out.println();
	}
	...
	private void printNodeInASCII(PrintWriter out, JadeNode node, int level, int m,
	        int maxm) {
	...
	private void printlnNodeWithNumberAndLabel(PrintWriter out, JadeNode node,
	        int level) {
		for (int i = 0; i < level - 1; i++) {
			if (umbrella[i]) {
				putCharAtLevel(out, i, '|');
...
*/
	...
	*/
	...
	private int printNH(PrintWriter out, JadeNode node, boolean printLengths,
