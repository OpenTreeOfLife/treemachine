#!/bin/sh
outputdir="simpleprob-with-answer-out"
dbdir=simpleprob-with-answer-test.db
inputdir=simpleprob
if test -d "${dbdir}"
then
...
java -jar target/treemachine-0.0.1-SNAPSHOT-jar-with-dependencies.jar addnewick "${inputdir}"/expected.tre F life expected "${dbdir}" >"${outputdir}"/addnewick-expected.log 2>&1 || exit
...
java -jar target/treemachine-0.0.1-SNAPSHOT-jar-with-dependencies.jar synthesizedrafttreelist_nodeid 1 expected,tree2,tree1,taxonomy "${dbdir}" >"${outputdir}"/synthesize.log 2>&1 || exit
