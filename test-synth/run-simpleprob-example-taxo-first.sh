#!/bin/sh
outputdir="simpleprob-taxofirst-out"
dbdir=simpleprob-taxofirst-test.db
inputdir=simpleprob
if test -d "${dbdir}"
then
...
java -jar target/treemachine-0.0.1-SNAPSHOT-jar-with-dependencies.jar synthesizedrafttreelist_nodeid 1 taxonomy,tree2,tree1 "${dbdir}" >"${outputdir}"/synthesize.log 2>&1 || exit
