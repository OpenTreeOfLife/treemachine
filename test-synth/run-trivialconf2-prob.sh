#!/bin/sh
outputdir="trivialconf2-out"
dbdir=trivialconf2-test.db
inputdir=trivialconf2
if test -d "${dbdir}"
then
...
java -jar target/treemachine-0.0.1-SNAPSHOT-jar-with-dependencies.jar synthesizedrafttreelist_nodeid 1 tree1,tree2,taxonomy "${dbdir}" >"${outputdir}"/synthesize.log 2>&1 || exit
