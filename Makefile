...
PYLIB=../../germinator/ws-tests
PYTHON=PYTHONPATH=$(PYLIB) python
...
neo4j: $(NEO)
$(NEO):
	curl http://files.opentreeoflife.org/neo4j/$(NEO)-unix.tar.gz >$(NEO)-unix.tar.gz
	tar xzf $(NEO)-unix.tar.gz
...
test-ws: .running
	../germinator/ws-tests/run_tests.sh -t . $(SETTINGS)
