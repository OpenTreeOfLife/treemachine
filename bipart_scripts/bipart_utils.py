...
"""
def process_pairs(start,pairs,bpslf,bpsrt,curlf,currt,path):
    lf = bpslf[start]
    rt = bpsrt[start]
    #print "here",lf,rt,curlf,currt
    ...
    for i in pairs[start]:
        ...
        x = process_pairs(i,pairs,bpslf,bpsrt,curlf,currt,path)
        if x != None:
            currt = currt.union(x[1])
    #print "\tnode to create: ",curlf,currt,path
...
if __name__ == "__main__":
    ...
    for i in bipart_pairs:
        print "generating for: ",i,bipartslf[i],bipartsrt[i]
        process_pairs(i,bipart_pairs,bipartslf,bipartsrt,set(),set(),[])
