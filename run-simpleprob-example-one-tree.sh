#!/bin/sh
outputdir="simpleprob-one-tree-out"
dbdir=simpleprob-one-tree-test.db
inputdir=simpleprob
if test -d "${dbdir}"
then
...
java -jar target/treemachine-0.0.1-SNAPSHOT-jar-with-dependencies.jar synthesizedrafttreelist_nodeid 1 tree2,taxonomy "${dbdir}" >"${outputdir}"/synthesize.log 2>&1 || exit
