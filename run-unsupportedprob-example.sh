#!/bin/sh
outputdir="unsupportedprob-out"
dbdir=unsupportedprob-test.db
inputdir=unsupportedprob
if test -d "${dbdir}"
then
...
java -jar target/treemachine-0.0.1-SNAPSHOT-jar-with-dependencies.jar addnewick "${inputdir}"/tree3.tre F life tree2 "${dbdir}" >"${outputdir}"/addnewick-tree3.log 2>&1 || exit
...
java -jar target/treemachine-0.0.1-SNAPSHOT-jar-with-dependencies.jar synthesizedrafttreelist_nodeid 1 tree1,tree2,tree3,taxonomy "${dbdir}" >"${outputdir}"/synthesize.log 2>&1 || exit
...
cat "${outputdir}"/addnewick-tree3.log
